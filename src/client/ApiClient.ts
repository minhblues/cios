...
export class ApiClient {
      ...
      if (errors.length > 0) {
        ...
        if (errors.length > 1) {
          const combinedMessage = errors
            .map(
              (err, index) =>
                `Request ${index}: ${err?.message || "Unknown error"}`
            )
            .join("; ");

          return [
            new ApiError(`Multiple requests failed: ${combinedMessage}`),
            null,
          ];
        }
  ...
  private async request<T>(
    ...
    try {
      ...
      if (
        mergedOptions.useXMLHttpRequest &&
        (mergedOptions.onUploadProgress || mergedOptions.onDownloadProgress)
      ) {
      ...
      const validateStatus =
        mergedOptions.validateStatus || this.defaultOptions.validateStatus;
      ...
      if (
        retries > 0 &&
        this.shouldRetry(error, mergedOptions.retryStatusCodes)
      ) {
      ...
      if (error instanceof Error) {
        ...
        processedError = await this.applyErrorInterceptors(
  ...
  private async retryRequest<T>(
    ...
    const adjustedDelay =
      delay * Math.pow(2, (this.defaultOptions.retries || 0) - retriesLeft);
